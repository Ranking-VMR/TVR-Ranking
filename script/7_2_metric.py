...
def recall_iou_ndcg(result_path, gt_path, T, K):
    ###### ------------------ #######
    ...
    gt_data["match_score"] = gt_data["match_score"] - 1
    gt_data[gt_data["match_score"] <= 0] = 0
    ...
    for i in tqdm(range(len(pred_data["VCMR"]))):
        ...
        true_scores = one_query_gts_back["match_score"].tolist()[:K]
        pred_scores = one_query_preds_df["pred_match_score"].tolist()[:K]
...
for K in [10, 20, 40]:
    for T in [0.3, 0.5, 0.7]:
        val_gt_path = "data/TVR_Ranking_raw/val_raw.jsonl"
        # val_result_path = "results/mul_train/top40/val_best_TVR-Ranking_val_predictions_VCMR.json"
        val_result_path = "results/mul_train/top1/best_val.json"
