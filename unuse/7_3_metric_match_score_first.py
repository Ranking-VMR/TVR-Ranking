...
for i in tqdm(range(len(pred_data["VCMR"]))):
    ...
    one_query_gts = gt_data[gt_data["qid"]==qid]
    # sort one_query_gts by "matchh_score"
    one_query_gts = one_query_gts.sort_values(by="match_score", ascending=False)

    # Prepare to update predictions with a match score
    predictions_with_scores = []

    # Iterate through each prediction
    for index, pred in one_query_preds_df.iterrows():
        pred_vid, pred_st, pred_ed = pred["vid"], pred["start_time"], pred["end_time"]
        matched_rows = one_query_gts[one_query_gts["vid"] == pred_vid]
        
        if matched_rows.empty:
            pred["pred_match_score"] = 0
        else:
            iou_scores = matched_rows.apply(lambda row: calculate_iou(pred_st, pred_ed, row["start_time"], row["end_time"]), axis=1)
            matched_rows = matched_rows.assign(iou=iou_scores) 
            
            max_iou_row = matched_rows.loc[matched_rows["iou"].idxmax()]
            pred["pred_match_score"] = max_iou_row["match_score"] if max_iou_row["iou"] > 0 else 0
        
        predictions_with_scores.append(pred)

    # Convert the predictions with scores back into a DataFrame
    one_query_preds_df = pd.DataFrame(predictions_with_scores)


    pred_scores = one_query_preds_df["pred_match_score"].tolist()
    true_scores = one_query_gts["match_score"].tolist()
    
    # Calculate NDCG for this query
    # ndcg_score = calculate_ndcg(true_scores, true_scores)
    ndcg_score = calculate_ndcg(pred_scores, true_scores[:len(pred_scores)])
